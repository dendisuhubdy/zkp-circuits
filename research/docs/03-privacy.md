...
| sBPF call: how much work it did | leaked, to within a factor of two, by `sha256_log_height` and the tier — and more than for other guests, because M4.4 hashes the ELF in-circuit: `sha256_log_height` is essentially `log2(program size / 64)`, so it bounds the *size of the program that ran*. A declared `program_hash` (`docs/04-guests.md`'s recommendation, which the cycle measurement makes necessary anyway) would remove that leak as a side effect, leaving `sha256_log_height` bounding only the instruction's own size |
