...
| M4 | EVM and sBPF guest interpreters, Keccak/SHA coprocessors (`docs/04-guests.md`) | an ERC-20 `transfer` and an SPL `Transfer` each prove under `R_exec` | in progress — M4.1 (compiled guests, flat-binary loader, `READ_INPUT` bound to `H_IN`) **done**; M4.2 (the Keccak-f\[1600\] chip, `KECCAK` syscall = 4, proof-declared keccak and memory heights, the keccak table optional per proof, the compiled `keccak256` guest) **done** — the suite stood at 223 tests then (222 pass, 1 ignored), M4.2 adding 38 of them (`tests/keccak.rs` (12, new), `tests/cheating.rs` (+15), `tests/e2e.rs` (+6), `tests/tables.rs` (+5)) and the 2026-09-12 audit port a further 18 (`tests/cheating.rs` (+11), `tests/asm.rs` (+5), `tests/e2e.rs` (+1), `tests/emulator.rs` (+1)); M4.3 (EVM interpreter) not started; M4.4 **in progress** — the SHA-256 chip and `SHA256` syscall = 5 are done (`tables::sha256`, `guests::sha256_demo`, the chip optional per proof and the verifier key a 5-tuple; `docs/superpowers/plans/2026-09-12-zkvm-m4-4.md` Tasks 1–4), the sBPF interpreter and the SPL `Transfer` exit test are not. The suite is 255 tests now (253 pass, 2 ignored), M4.4's chip tasks adding 32 of them (`tests/sha256.rs` (14, new), `tests/cheating.rs` (+11), `tests/e2e.rs` (+4), `tests/emulator.rs` (+2), `tests/tables.rs` (+1)) |
