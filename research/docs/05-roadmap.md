...
7. **M4.3's ERC-20 `transfer` proves at tier 18, not the plan's ≤ 16 (spec
   ...
   does not fit the development machine (~25 GB resident at tier 18, OOM-killed on
   48 GB), so it is an `#[ignore]`d test and the in-suite EVM proof is a smaller
   call through the same binary at tier 16.
