...
script `deploy/sync-zkvm.sh` patches on two anchors this set moved:
`Machine::verifier_key`'s **exact signature line**, which gained a sixth
parameter (`public_log_height`), and `pv::NUM`, now 34. Both must be updated
in the same re-vendoring; the script fails loudly rather than mispatching
silently, and it already carries the message for it. The recursion VM (M5) is
