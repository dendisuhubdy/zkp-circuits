...
//!   ignoring `CFLAGS`, with `--no-default-config`, and prints the clang's `--version` line as a
//!   cargo warning. It refuses a clang other than [`CLANG_VERSION`] unless
//!   `RAND_GUEST_CLANG_UNPINNED=1`, which builds with a loud warning that `hc` will not match. Rust's `compiler_builtins`
...
/// The clang the generated `build.rs` accepts: rand-guest's pin (`CLANG_VERSION` in
/// `rand-guest/src/build.rs` on the sBPF branch, which this branch receives by rebase), with the
/// same `RAND_GUEST_CLANG_UNPINNED=1` override. `hc` depends on the C compiler, so a verifier
/// rebuilding an image must use this clang to reproduce it.
...
/// rand-guest's clang pin: `hc` is published for this clang's output. Any other version is
/// refused unless RAND_GUEST_CLANG_UNPINNED=1, and then the build warns that hc will not match
/// published images.
const CLANG_VERSION: &str = "@CLANG@";

/// Asserts `clang` is the pinned version, and prints its `--version` line as a cargo warning.
fn assert_pinned(clang: &Path) {
    ...
    println!("cargo:warning=evm2rv: clang {first} ({})", clang.display());
    let version = first
...
fn main() {
    ...
    b.compiler(&clang)
        ...
        // No clang configuration file: Homebrew's LLVM ships per-target `.cfg` files, and one
        // found for some target would add flags this list does not name.
...
//! input vector's code, as for the interpreter. Before anything runs, the code guard checks that
//! code is the one `contract.c` was translated from. Do not edit — re-run evm2rv.
...
/// Words per `POSEIDON2` call (`evm2rv::guard::DIGEST_CHUNK`).
const CHUNK: usize = 4096;

/// The code guard's buffer: one `POSEIDON2` call's words.
...
extern "C" {
    ...
    fn evm_entry();
    /// The code guard's constant (`contract.c`): `evm2rv::guard::code_digest` of the bytecode the
    /// contract was translated from, 8 words.
    fn evm_code_digest() -> *const u32;
}
...
/// The code guard: whether `code` (the input vector's) is the code this image was translated
/// from. The translated logic is baked into the image, but `CODECOPY` and `CODESIZE` read the
/// input vector's code, so a different code would run this logic over someone else's bytes. The
/// message is `evm2rv::guard::code_words` (the length in bytes, then the code four bytes per word
/// little-endian, the last zero-padded), hashed as `evm2rv::guard::chained_digest` hashes it: in
/// `CHUNK`-word `POSEIDON2` calls, each after the first beginning with the previous digest. No
/// slicing, so no panic path.
fn is_the_translated_code(code: &[u8]) -> bool {
    let buf = addr_of_mut!(HASH_BUF) as *mut u32;
    let (ptr, len) = (code.as_ptr(), code.len());
    let whole = len / 4;
    ...
    unsafe {
        ...
        while i < whole {
            ...
            let n = (CHUNK - fill).min(whole - i);
            let src = ptr.add(4 * i) as *const u32;
            let dst = buf.add(fill);
            if aligned {
        ...
        if rest != 0 {
            ...
            let mut w = 0u32;
            for k in 0..rest {
                w |= (*ptr.add(4 * whole + k) as u32) << (8 * k);
            }
            buf.add(fill).write(w);
            fill += 1;
        }
        poseidon2(buf, fill);
        let want = evm_code_digest();
        let mut same = true;
        for k in 0..8 {
            same &= *buf.add(k) == *want.add(k);
        }
        same
    }
}
...
fn translated(
    ...
    unsafe {
        ...
        // The interpreter's `pre_halt`: nothing runs, nothing is spent. The code guard refuses the
        // same way — before any translated code runs, so the call never began (accepted
        // divergence #3: the interpreter would run the other code).
        if pre != 0 || !is_the_translated_code(code) {
