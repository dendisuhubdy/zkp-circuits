...
use recursion::emulator::PermEvent;
use recursion::isa::F;
...
fn the_chips_output_equals_the_reference_on_one_thousand_random_states() {
    ...
    for (i, &(clk, ev)) in events.iter().enumerate() {
        ...
        assert_eq!(output, rand_zkvm::tables::poseidon2::permute_scalar(ev.input),
    ...
    assert_eq!(out, rand_zkvm::tables::poseidon2::permute_scalar([F::ZERO; 8]));
...
fn the_round_constants_are_the_machines_own_draw() {
    ...
    // shares all come from `rand_zkvm::tables::poseidon2`.
    let rc = rand_zkvm::tables::poseidon2::round_constants();
    ...
    for _ in 0..20 {
        ...
        assert_eq!(
            rand_zkvm::tables::poseidon2::permute_scalar(s),
            rand_zkvm::hash::permute_state(s),
