...
use crate::isa::F;
use crate::shape::{inner_vk_digest, InnerKey, InnerShape, PV_INSTANCE};
...
/// (4 elements), `N`, then `N × 34` inner public values (constraint set 6's `pv::NUM = 34`,
/// `PUB0..7` included — dropping `HC0..7` would let an aggregate accept a proof of a different
/// guest). The fullnode recomputes this list from the covered bundles' public fields and its
/// registered `hc`, hashes it with [`public_digest`], and compares.
pub fn interface_words(shape: &InnerShape, key: &InnerKey, pvs: &[Vec<u64>]) -> Vec<F> {
    ...
    for pv in pvs {
        assert_eq!(
            pv.len(),
            shape.num_public_values[PV_INSTANCE],
            "every inner proof carries exactly pv::NUM public values"
        );
