...
| `evm2rv` | EVM (Solidity) bytecode → RV32 ahead-of-time translator | `evm-core`, `evm-rt`, `rand-guest` | native execution vs. interpretation for the same ABI; an image that refuses any code but its source, so `hc` binds the code; `gas_used` is in the `Outcome` but not bound by `public_output`, so it matters on chain only through status; a real ERC-20 walkthrough, word for word against the interpreter |
