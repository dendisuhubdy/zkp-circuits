...
- The C is compiled with exactly the toolchain's flags (`--target=riscv32-unknown-none-elf -march=rv32im -mabi=ilp32 -mno-relax -nostdlib -ffreestanding -fno-builtin -Os`); no `rv32imc`. **Named exception (Task 4)**: the emitted translated functions carry `__attribute__((minsize))` in the source (the flags themselves are unchanged); it is part of what fits a whole SPL program under the 65 535-word cap.
- The instruction-limit check is per basic block at the block head. **Amended 2026-09-18** (Task 3 review): it halts `InstructionLimit` at the head of the block that would cross the limit; the halt kind may differ from the interpreter's only in that block (status and public words are equal there). Task 4 (final after its review): a block may skip its check only if it does not end in exit/call/callx, has no self-loop, and every in-function neighbour checks — the next block's check then halts, and the guarantee is the same.
