...
- The instruction-limit check is per basic block at the block head. **Amended 2026-09-18** (Task 3 review): it halts `InstructionLimit` at the head of the block that would cross the limit; the halt kind may differ from the interpreter's only in that block (status and public words are equal there). Task 4 (pending review) also lets a block defer its check to its successors' heads under the conditions spec §3 states, which keeps the same guarantee.
