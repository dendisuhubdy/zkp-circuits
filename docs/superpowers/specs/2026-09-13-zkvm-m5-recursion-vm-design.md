...
Status: **design approved in conversation 2026-09-13; spec reviewed and approved by the user 2026-09-14. M5.1 built (2026-09-14), against constraint set 6: 50 real bundle proofs verified in-circuit at 5 682 847 cpu rows per inner proof — over the 2^19 point; Task 7's precompiles provably cannot close the gap (~3%), so it did not run. See `recursion/docs/00-recursion-vm.md`. M5.2 planned 2026-09-14 (`docs/superpowers/plans/2026-09-14-zkvm-m5-2.md`): the measurement forces a preprocessed program table (no in-circuit digest), `TIERS = [8..22]` re-cut with the exit at 21 on a ≥ 64 GB machine, and `REDUCE`/`SPONGE` precompiles staged behind allocator liveness. M5.2 built 2026-09-15 (the machine, three row cuts, the exit at tier 21 in plan). M5.3 planned 2026-09-15 (`docs/superpowers/plans/2026-09-15-zkvm-m5-3.md`): one N-generic aggregate program per shape with the interface digest over `[vk ‖ N ‖ 34·N]`; the exit is the N=3 test-profile twin on the dev machine; production N≥2 is tier 22/23 at ~97–134 GB of oracle, deferred to M5.4's GPU backend. M5.3 built 2026-09-15 (the N-proof tape, the N-generic aggregate program with its counted loop and runtime-length interface sponge, the chain-facing `aggregate`/`verify_aggregate` API, the refusal suite; measured rows N=1 441 782 / N=2 883 240 / N=3 1 324 694 at the test profile, production N=1 1 968 758; the exit's N=3 twin jetsam-deferred on the shared dev box with the N=1 tier-19 aggregate the completed in-scope proof; the 2026-09-15 ruling: production-profile proofs execute on a ≥ 64 GB machine after chain-side aggregation lands). M5.4 planned 2026-09-15 (`docs/superpowers/plans/2026-09-15-zkvm-m5-4.md`): the CUDA backend as a config-level mirror of the RV32 split (no new kernels), the tier-23 rung, N re-measured on the GPU node, and the self-verifier deferred with its measured requirement.**
